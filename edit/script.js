...
function init() {
    ...
    if (btnExitGame) {
        btnExitGame.addEventListener('click', () => {
            // ✨ 採用您的「簡單規則」：優先檢查網址是否有 bank 參數
            const urlParams = new URLSearchParams(window.location.search);
            if (urlParams.has('bank')) {
                switchMode('bank');
            } else {
                // 如果沒有題庫參數，則回歸原本的來源記憶或預設回表格
    ...
    let hasBankAutoLoaded = false; 

    // ✨ 1. 優先處理題庫直接載入 (?bank=xxx)
    if (urlParams.has('bank') && window.presetBank) {
        const bankId = urlParams.get('bank');
        const bankIdx = window.presetBank.findIndex(p => p.id === bankId);
        
        if (bankIdx !== -1) {
            // 抓取網址上的目標模式 (例如 mode=sorting)
            let forcedMode = null;
            const urlModeParam = urlParams.get('mode');
            if (urlModeParam && urlModeParam !== 'bank') {
                forcedMode = urlModeParam === 'sheet' ? 'table' : (urlModeParam === 'txt' ? 'text' : urlModeParam);
            }
            
            // ✨ 將網址上的目標模式一併傳給題庫引擎
            playPresetBank(bankIdx, forcedMode); 
            hasBankAutoLoaded = true;
        }
    }

    // ✨ 2. 若沒載入題庫，才執行原有的 mode 切換邏輯
    if (!hasBankAutoLoaded) {
...
function updateUrlModeParam(internalMode) {
    ...
    // ✨ 2. 新增：智慧同步題庫 ID (bank 參數)
    if (window.currentLoadedBank && window.currentLoadedBank.id) {
        if (newUrl.searchParams.get('bank') !== window.currentLoadedBank.id) {
            newUrl.searchParams.set('bank', window.currentLoadedBank.id);
            urlChanged = true;
        }
    } else {
        // 如果目前沒有載入題庫 (例如使用者自己打字)，則清除 bank 參數保持網址乾淨
        if (newUrl.searchParams.has('bank')) {
...
/**
 * 1. 更新網址參數 (當點擊進入資料夾時呼叫)
 * @param {string} folderName - 中文資料夾名稱
 */
function updateFolderUrl(folderName) {
    const url = new URL(window.location);
    // 使用 encodeURIComponent 將中文安全轉換為網址格式
    url.searchParams.set('folder', encodeURIComponent(folderName));
    // pushState 可以更改網址列，但不會觸發網頁重新整理！
    window.history.pushState({}, '', url);
}

/**
 * 2. 清除網址參數 (當點擊「返回」或回到主目錄時呼叫)
 */
function clearFolderUrl() {
    const url = new URL(window.location);
...
/**
 * 3. 檢查網址是否有資料夾參數 (通常在網頁一載入時呼叫)
 * @returns {string|null} - 傳回解碼後的資料夾名稱，若無則傳回 null
 */
function getFolderFromUrl() {
    ...
    if (folderParam) {
        // 將網址參數解碼回中文
        return decodeURIComponent(folderParam);
    }
    return null;
}
...
function switchMode(mode, isForce = false, dataOverride = null, configOverride = null) {
    ...
    if (!gameModes.includes(currentMode) && currentMode !== 'arena' && currentMode !== 'live') {
        if (!dataOverride) {
            window.originModeForGame = currentMode;
        }
    }
    ...
    // 遊戲/連線模式不存入 LocalStorage
    if (!gameModes.includes(mode) && mode !== 'arena' && mode !== 'live') {
    ...
    const containers = {
        ...
        table: document.getElementById('tableModeContainer'),
        chat: document.getElementById('chatModeContainer'),
        game: document.getElementById('gameModeContainer'),
        arena: document.getElementById('arenaModeContainer'),
        bank: document.getElementById('bankModeContainer')
    };
    Object.values(containers).forEach(c => { if(c) { c.style.display = 'none'; c.classList.add('hidden'); } });
    ...
    const btnToggleLineNumbers = document.getElementById('btnToggleLineNumbersTextMode');

    if (ddLanguageTools) ddLanguageTools.classList.remove('hidden');
	...
	if (mode === 'arena' || mode === 'live' || gameModes.includes(mode)) {
        ...
        window.lastGameData = rawDataText;

        const configs = configOverride || window.currentGameConfigs || { matchPairs: [{ id: 1, termCol: 0, defCol: 1, customName: '' }] };
        ...
        } else if (typeof window.launchGameMode === 'function') {
        ...
        applyFreeze();
        containers.table.style.display = 'block';
        if (tableControls) tableControls.classList.replace('hidden', 'flex');
        if (ddTextTool) ddTextTool.classList.add('hidden');
        if (btnToggleLineNumbers) btnToggleLineNumbers.classList.add('hidden');

    } else if (mode === 'chat') {
        containers.chat.classList.remove('hidden');
        containers.chat.style.display = 'flex';
        if (chatControls) chatControls.classList.replace('hidden', 'flex');
        if (ddLanguageTools) ddLanguageTools.classList.add('hidden');
        ...
        if (typeof window.renderChatFromText === 'function') window.renderChatFromText(editor.value);

    } else {
        containers.text.style.display = 'flex';
        if (ddTextTool) ddTextTool.classList.remove('hidden');
        if (btnToggleLineNumbers) btnToggleLineNumbers.classList.remove('hidden');
        updateLineNumbers();
    }
...
document.addEventListener('keydown', (e) => {
    ...
    if (currentMode === 'table') {
            const isInputFocus = document.activeElement && ['INPUT', 'TEXTAREA', 'SELECT'].includes(document.activeElement.tagName);
            
            if (isInputFocus && e.key !== 'Tab') {
                return;
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const SHORT_NAMES = {
        'chinese': '華', 'sixian': '四', 'sixiannan': '南',
        'hailu': '海', 'dapu': '埔', 'raoping': '饒',
        'kasu': '詔', 'holo': '台', 'jinmen': '金',
        'matsu': '福', 'pinyin': '拼', 'segment': '詞'
    };
    ...
    // --- 2. UI 邏輯與膠囊按鈕 ---
    function initDropdowns() {
        sourceBtn.textContent = SHORT_NAMES[transState.source] || '華';
        targetBtn.textContent = SHORT_NAMES[transState.target] || '詔';
	...
	function setLanguage(type, value) {
        ...
        if (type === 'source') {
            sourceBtn.textContent = SHORT_NAMES[value] || '華';
            localStorage.setItem(STORE_KEY_SOURCE, value);
        } else if (type === 'target') {
            targetBtn.textContent = SHORT_NAMES[value] || '詔';
            localStorage.setItem(STORE_KEY_TARGET, value);
        }
        updatePivotOptions(); 
    ...
    function updatePivotOptions() {
        ...
        // 建立選單項目的輔助函數 (加入打勾圖示)
        const createPivotItem = (value, text, isSelected) => {
            const item = document.createElement('div');
            item.className = 'dropdown-item flex items-center justify-between gap-3'; 
            item.dataset.value = value;
            
            item.innerHTML = `
                <span>${text}</span>
                <span class="material-symbols-outlined text-[18px] text-blue-600 font-bold" style="opacity: ${isSelected ? '1' : '0'};">check</span>
            `;
            
            item.onclick = (e) => {
                transState.pivot = value; 
                updatePivotLabel(); 
                e.target.closest('.dropdown-list').classList.remove('show');
                updatePivotOptions(); // 重新渲染選單以更新打勾狀態
            };
            return item;
        };

        if (canDirect) {
            pivotList.appendChild(createPivotItem('direct', '直達', transState.pivot === 'direct'));
        }

        // 處理選項文字的渲染
        possiblePivots.forEach(pivotKey => {
            const names = pivotKey.split(',').map(p => LANGUAGES[p].name);
            const isSelected = transState.pivot === pivotKey;
            pivotList.appendChild(createPivotItem(pivotKey, names.join(' > '), isSelected));
        });
    ...
    async function performConversion(actionMode) {
		...
		try {
            ...
            if (['exact', 'all', 'fuzzy'].includes(actionMode)) {
                ...
                // 文字模式處理：行後加 TAB
                if (!isTableMode && editor && currentMode === 'text') { // 🌟 修正：確保只有文字模式才寫入 editor
                ...
                // 🌟 新增：支援 Live、Arena、Chat 模式下的進階比對與便利貼
                else if (currentMode !== 'text' && currentMode !== 'table') {
                    ...
                    // 2. 以下保留原本處理輸入框與純文字反白的邏輯
                    let activeEl = document.activeElement;
                    let isInputTarget = activeEl && (activeEl.tagName === 'INPUT' || activeEl.tagName === 'TEXTAREA');
                    // 引用追蹤器找回輸入框
                    if (!isInputTarget && window.lastFocusedInput && document.body.contains(window.lastFocusedInput)) {
                    ...
                    if (isInputTarget) {
                        ...
                        let newLines = lines.map(line => {
                            ...
                            if (!word) return line;
                            let matches = getMatches(word);
                            return matches.length > 0 ? line + '\t' + matches.join('、') : line;
                        });
                        let newText = newLines.join('\n');
                        if (start !== end) activeEl.setRangeText(newText, start, end, 'select');
                        else activeEl.value = newText;
...
// ========== 以下是對話模式與翻譯系統的共用邏輯 ==========
    ...
    const chatToggleDelete = document.getElementById('chatToggleDelete');
    const chatToggleLangName = document.getElementById('chatToggleLangName'); // 補回語言開關
    ...
    function updateChatDisplaySettings() {
        ...
        if(chatToggleDelete) localStorage.setItem('wesing-chat-show-delete', chatToggleDelete.checked);
        if(chatToggleLangName) localStorage.setItem('wesing-chat-show-lang-name', chatToggleLangName.checked); // 記憶語言開關
    }
    ...
    if (chatToggleNumber && chatToggleEmoji) {
        ...
        // 讀取語言開關的記憶
        const savedShowLangName = localStorage.getItem('wesing-chat-show-lang-name');
        if (savedShowLangName !== null && chatToggleLangName) chatToggleLangName.checked = (savedShowLangName === 'true');
        ...
        if (chatToggleDelete) chatToggleDelete.addEventListener('change', updateChatDisplaySettings);
        if (chatToggleLangName) chatToggleLangName.addEventListener('change', updateChatDisplaySettings); // 綁定事件
    ...
    if (chatInput && btnSendChat) {
        ...
        function initChatDropdowns() {
            // 👇 修改：預設讀取 SHORT_NAMES 單字
            document.getElementById('chat-source-btn').textContent = SHORT_NAMES[chatTransState.source] || '華';
            document.getElementById('chat-target-btn').textContent = SHORT_NAMES[chatTransState.target] || '詔';

            document.querySelectorAll('#chat-dropdown-source .dropdown-item').forEach(item => {
                item.addEventListener('click', (e) => {
                    chatTransState.source = item.dataset.value;
                    // 👇 修改：按鈕顯示單字
                    document.getElementById('chat-source-btn').textContent = SHORT_NAMES[item.dataset.value] || '華';
            ...
            document.querySelectorAll('#chat-dropdown-target .dropdown-item').forEach(item => {
                item.addEventListener('click', (e) => {
                    ...
                    document.getElementById('chat-target-btn').textContent = SHORT_NAMES[item.dataset.value] || '詔'; 
        ...
        function updateChatPivotOptions() {
            ...
            // 建立選單項目的輔助函數 (加入打勾圖示)
            const createChatPivotItem = (value, text, isSelected) => {
                const item = document.createElement('div');
                item.className = 'dropdown-item flex items-center justify-between gap-3'; 
                item.dataset.value = value;
                ...
                item.onclick = (e) => {
                    chatTransState.pivot = value; 
                    pivotLabel.style.display = 'none'; // 按鈕永遠不顯示文字
                    e.target.closest('.dropdown-list').classList.remove('show');
                    preloadChatDictionaries();
                    updateChatPivotOptions(); // 重新渲染選單以更新打勾狀態
                };
                return item;
            };

            if (canDirect) {
                pivotList.appendChild(createChatPivotItem('direct', '直達', chatTransState.pivot === 'direct'));
            }

            possiblePivots.forEach(pKey => {
                const names = pKey.split(',').map(p => LANGUAGES[p].name);
                const isSelected = chatTransState.pivot === pKey;
                pivotList.appendChild(createChatPivotItem(pKey, names.join(' > '), isSelected));
            });

            // 設定預設值
            if (canDirect) {
                if (chatTransState.pivot !== 'direct' && !possiblePivots.includes(chatTransState.pivot)) {
                    chatTransState.pivot = 'direct';
                }
                if (!chatTransState.pivot) chatTransState.pivot = 'direct';
            } else {
                if (possiblePivots.length > 0) {
                    if (chatTransState.pivot === 'direct' || !possiblePivots.includes(chatTransState.pivot)) {
            ...
            // 將目前的狀態寫入 title，滑鼠懸浮時可以看
            if (chatTransState.pivot === 'direct' || !chatTransState.pivot) {
                pivotWrapper.title = "設定中介語言 (目前：直達)";
            } else {
        ...
        // 2. 聊天互動與發送
        const btnClearChatInput = document.getElementById('btnClearChatInput'); // 取得清除按鈕
        ...
        chatInput.addEventListener('input', function() {
            ...
            // 👇 動態顯示/隱藏清除按鈕
            if (btnClearChatInput) {
                if (this.value.length > 0) {
                    btnClearChatInput.classList.remove('hidden');
                } else {
                    btnClearChatInput.classList.add('hidden');
                }
            }
        });
        ...
        // 👇 綁定清除按鈕點擊事件
        if (btnClearChatInput) {
            btnClearChatInput.addEventListener('click', () => {
                chatInput.value = '';
                chatInput.style.height = 'auto';
                btnClearChatInput.classList.add('hidden');
                chatInput.focus(); // 清除後保持游標在輸入框
            });
        }
        ...
        async function handleSendChat() {
            ...
            try {
                ...
                let effectiveOutputLang = target;
                if (effectiveOutputLang === 'pinyin' || effectiveOutputLang === 'segment') {
                    effectiveOutputLang = source;
                }
                const langName = LANGUAGES[effectiveOutputLang] ? LANGUAGES[effectiveOutputLang].name : '';
                const showLangName = document.getElementById('chatToggleLangName') && document.getElementById('chatToggleLangName').checked;

                // 如果有開啟，且有文字、有語言名稱，就組合在一起
                if (showLangName && currentText && langName) {
                    currentText = `${langName}：${currentText}`;
                }
        ...
        function appendChatMessage(text, isUser, pinyinText = '') {
            ...
            if (!isUser) {
                ...
                if (text) {
                    // 掃描所有語言，看看文字是否以「語言名稱：」開頭
                    for (const key in LANGUAGES) {
                        const langName = LANGUAGES[key].name;
                        // 同時支援全形與半形的冒號
                        if (text.startsWith(langName + '：') || text.startsWith(langName + ':')) {
                            matchedLangKey = key;
                            break; // 找到就停止掃描
                        }
                    }
                ...
                // 如果文字自帶語言名稱前綴，優先使用該語言的 Emoji；否則使用系統當前的輸出語言
                const finalEmojiLang = matchedLangKey || effectiveLang;
                const sysEmoji = langEmojis[finalEmojiLang] || '🤖';
            ...
            actions.className = 'chat-actions flex gap-2 md:gap-0.5 mb-1 opacity-0 md:group-hover:opacity-100 transition-opacity duration-200';
            ...
            wrapper.addEventListener('click', (e) => {
                ...
                if (window.matchMedia && window.matchMedia('(pointer: coarse)').matches) {
                    ...
                    document.querySelectorAll('.chat-actions').forEach(act => {
                        if (act !== actions) {
                            act.classList.remove('opacity-100');
                            act.classList.add('opacity-0');
                        }
                    });
...
document.addEventListener('click', (e) => {
    ...
    if (typeof WebIME !== 'undefined') {
        ...
        // 單純判斷我們自己的變數
        if (!window.isMyImeLoaded) {
            // 【執行載入】
            if (typeof WebIME.imeInit === 'function') {
                WebIME.imeInit({});
            }
            window.isMyImeLoaded = true; // 更新狀態為已載入
            showToast('✅ 烏衣行輸入法已載入！');
        } 
        else {
            // 【執行卸載】
            ...
            window.isMyImeLoaded = false; // 更新狀態為未載入
            showToast('❌ 烏衣行輸入法已卸載');
        }

        // 載入或卸載後，將游標精準對焦回輸入框，讓體驗不中斷
        setTimeout(() => {
            if (currentMode === 'text') {
                const editor = document.getElementById('editor');
        ...
        if (index === activeSheetIndex) {
            ...
            menuBtn.onclick = (e) => {
               ...
               let leftPos = rect.left;
                const menuWidth = tabMenu.offsetWidth;
                
                // 如果選單的右側會超出螢幕右邊界，就把選單強制往左推
                if (leftPos + menuWidth > window.innerWidth) {
                    leftPos = window.innerWidth - menuWidth - 10; // 距離右邊緣保留 10px 安全距離
                }
                // 確保左側也不會超出螢幕（雙重保險）
                tabMenu.style.left = `${Math.max(5, leftPos)}px`; 
...
if (btnStartGame) {
    ...
    if (btnStartGame) {
		// ✨ 遊戲專用分享邏輯 (供學習模式與連線模式呼叫)
    window.generateGameShareLink = async function(targetMode) {
        let shareUrl = new URL(window.location.href);
        shareUrl.searchParams.delete('mode');
        shareUrl.searchParams.delete('bank');
        shareUrl.searchParams.delete('id');
        shareUrl.searchParams.delete('ime');

        shareUrl.searchParams.set('mode', targetMode);

        if (window.currentLoadedBank) {
            // 如果沒改過資料，直接分享 bank ID，網址超短！
            shareUrl.searchParams.set('bank', window.currentLoadedBank.id);
            copyUrlAndToast(shareUrl.href);
        } else {
            // 如果是被編輯過的資料，使用 Firebase 儲存並回傳短 ID
            showToast('⏳ 正在產生雲端連結，請稍候...');
            let currentContent = currentMode === 'text' ? editor.value : extractTextFromTable();
            if (!currentContent) return showToast('⚠️ 沒有內容可以分享');
            const compressedData = compressDataForUrl(currentContent);
            const docId = await saveToFirebase(compressedData);
            if (!docId) return showToast('❌ 產生網址失敗');
            shareUrl.searchParams.set('id', docId);
            copyUrlAndToast(shareUrl.href);
        }
    };

    function copyUrlAndToast(url) {
        if (navigator.clipboard && window.isSecureContext) {
            navigator.clipboard.writeText(url).then(() => showToast('🔗 遊戲直接連結已複製！開啟後將直接進入遊戲。'));
        } else {
            showPrompt('請手動複製以下網址：', url, () => {});
        }
    }

    const btnShareGame = document.getElementById('btnShareGame');
    if (btnShareGame) {
        btnShareGame.addEventListener('click', () => {
            const selectedGame = document.querySelector('input[name="gameType"]:checked').value;
            window.generateGameShareLink(selectedGame);
        });
    }
...
function renderGameMatchPairs() {
    ...
    // 將原本的 selectedGameType 改為 effectiveConfigMode
    if (effectiveConfigMode === 'choice') {
        // ========== 選擇題專屬介面 ==========
...
// ✨ 新增：當連線模式的「題型來源」改變時，即時重繪下方的欄位設定
...
const bankSidebar = document.getElementById('bankSidebar');
const bankSidebarOverlay = document.getElementById('bankSidebarOverlay');
const bankFoldersGrid = document.getElementById('bankFoldersGrid');
const bankSidebarMenu = document.getElementById('bankSidebarMenu');
...
// 自動反轉對照表，供讀取網址時使用
const reverseCategoryAliasMap = Object.fromEntries(
    Object.entries(categoryAliasMap).map(([key, value]) => [value, key])
);

// 2. 輔助函數：真實且簡短的 UTF-8 Base64 轉換引擎 (Url Safe)
function encodeBase64UrlSafe(str) {
    // 1. 使用 TextEncoder 將中文直接轉為真實的 UTF-8 位元組 (不會暴增長度)
    const bytes = new TextEncoder().encode(str);
    let binaryStr = '';
    for (let i = 0; i < bytes.length; i++) {
        binaryStr += String.fromCharCode(bytes[i]);
    }
    // 2. 轉為 Base64 並替換網址安全符號
    return btoa(binaryStr).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}
...
function decodeBase64UrlSafe(base64Str) {
    try {
        ...
        // 3. 使用 TextDecoder 解碼回中文
        return new TextDecoder().decode(bytes);
    } catch (e) {
        return base64Str; // 解碼失敗退回原字串以防崩潰
    }
}
...
// 1. 初始化題庫資料與篩選器
function initBankCenter() {
    if (!window.presetBank) return;
    window.presetBank.forEach((p, idx) => p._originalIndex = idx);
    
    // ✨ 補全：從網址讀取分類參數 (自動處理 英文/Base64)
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('cat')) {
        const catParam = urlParams.get('cat');
        let realCat = reverseCategoryAliasMap[catParam];
        if (!realCat) {
            realCat = decodeBase64UrlSafe(catParam);
        }
        
        const validCategories = [...new Set(window.presetBank.map(p => p.folder))];
        if (validCategories.includes(realCat)) {
            bankState.category = realCat;
        }
    }

    // 初始化題型下拉選單
    const types = [...new Set(window.presetBank.map(p => p.type))].filter(Boolean);
    const filter = document.getElementById('bankTypeFilter');
    if (filter) {
        filter.innerHTML = '<option value="ALL">所有題型</option>' + 
...
function renderBankSidebar() {
    const categories = [...new Set(window.presetBank.map(p => p.folder))].filter(Boolean).sort();
    if (!bankSidebarMenu) return;

    bankSidebarMenu.innerHTML = `
        <button class="w-full text-left px-4 py-2.5 rounded-full flex items-center gap-3 transition-all ${bankState.category === 'ALL' ? 'bg-indigo-100 text-indigo-800 font-bold' : 'text-gray-600 hover:bg-gray-200'}" onclick="setBankCategory('ALL')">
            <span class="material-symbols-outlined text-[20px]">database</span> 總題庫
        </button>
        <div class="text-[11px] font-bold text-gray-400 mt-4 mb-2 px-4 tracking-wider uppercase">分類資料夾</div>
    ` + categories.map(cat => `
        <button class="w-full text-left px-4 py-2 rounded-full flex items-center gap-3 transition-all ${bankState.category === cat ? 'bg-indigo-50 text-indigo-700 font-bold' : 'text-gray-600 hover:bg-gray-100'}" onclick="setBankCategory('${cat}')">
            <span class="material-symbols-outlined text-[18px]">${bankState.category === cat ? 'folder_open' : 'folder'}</span>
            <span class="truncate">${cat}</span>
        </button>
    `).join('');
}

// 3. 設定分類路徑並重新渲染 (包含回到頂端)
window.setBankCategory = function(cat) {
    bankState.category = cat;
    bankState.search = '';
    if (bankSearchInput) bankSearchInput.value = '';
    if (btnClearBankSearch) btnClearBankSearch.classList.add('hidden');
    ...
    if (cat === 'ALL') {
        ...
        const alias = categoryAliasMap[cat];
        if (alias) {
            newUrl.searchParams.set('cat', alias); 
        } else {
            newUrl.searchParams.set('cat', encodeBase64UrlSafe(cat)); 
        }
    }
    window.history.replaceState({}, '', newUrl);
...
function toggleBankSidebar() {
    if (window.innerWidth >= 768) {
        // 電腦版：縮放寬度
        const isCollapsed = bankSidebar.classList.contains('md:w-0');
        if (!isCollapsed) {
            bankSidebar.classList.replace('w-64', 'md:w-0');
            bankSidebar.classList.add('border-none');
        } else {
            bankSidebar.classList.replace('md:w-0', 'w-64');
            bankSidebar.classList.remove('border-none');
        }
    } else {
        // 手機版：遮罩滑出
        if (bankSidebarOverlay.classList.contains('hidden')) {
            bankSidebarOverlay.classList.remove('hidden');
            setTimeout(() => {
                bankSidebarOverlay.classList.remove('opacity-0');
                bankSidebar.classList.remove('-translate-x-full');
            }, 10);
        } else {
            window.closeBankSidebar();
        }
    }
}

window.closeBankSidebar = function() {
    if (window.innerWidth >= 768) return; // 電腦版點選項不自動收合
    if (bankSidebarOverlay) bankSidebarOverlay.classList.add('opacity-0');
    if (bankSidebar) bankSidebar.classList.add('-translate-x-full');
    setTimeout(() => {
        if (bankSidebarOverlay) bankSidebarOverlay.classList.add('hidden');
    }, 300);
};

// 5. 內容渲染 (麵包屑 + 網格 + 清單)
window.renderBankContent = function() {
    const breadcrumbs = document.getElementById('bankBreadcrumbs');
    const foldersView = document.getElementById('bankFoldersView');
    const filesView = document.getElementById('bankFilesView');

    // 麵包屑導覽 (層次引導)
    if (breadcrumbs) {
        breadcrumbs.innerHTML = bankState.category === 'ALL' 
            ? '<b>總題庫</b>' 
            : `<span class="text-gray-400 cursor-pointer hover:underline" onclick="setBankCategory('ALL')">總題庫</span> <span class="text-gray-300 mx-2">/</span> <b>${bankState.category}</b>`;
    }

    // 過濾邏輯
    let filtered = window.presetBank.filter(p => {
        const mSearch = p.name.toLowerCase().includes(bankState.search.toLowerCase());
        const mCat = bankState.category === 'ALL' || p.folder === bankState.category;
        const mType = bankState.type === 'ALL' || p.type === bankState.type;
        return mSearch && mCat && mType;
    });

    // 排序邏輯
    filtered.sort((a, b) => {
        let valA = a[bankState.sortCol] || '';
        let valB = b[bankState.sortCol] || '';
        if (bankState.sortCol === 'count') { valA = getQCount(a.data); valB = getQCount(b.data); }
    ...
    // 在「總題庫」且沒搜尋時，顯示分類資料夾
    if (bankState.category === 'ALL' && !bankState.search) {
        foldersView.classList.remove('hidden');
        const cats = [...new Set(window.presetBank.map(p => p.folder))].filter(Boolean).sort();
        bankFoldersGrid.innerHTML = cats.map(c => `
            <div class="bg-[#f0f4f9] hover:bg-[#e1e5ea] p-4 rounded-xl flex items-center gap-3 cursor-pointer transition-all border border-transparent hover:border-gray-300 group" onclick="setBankCategory('${c}')">
                <span class="material-symbols-outlined text-indigo-500 text-3xl group-hover:scale-110 transition-transform">folder</span>
...
if (bankSearchInput) {
    bankSearchInput.addEventListener('input', (e) => {
        ...
        if (btnClearBankSearch) btnClearBankSearch.classList.toggle('hidden', e.target.value === '');
        renderBankContent();
    });
    if (btnClearBankSearch) {
...
document.getElementById('btnOpenBankSidebar')?.addEventListener('click', toggleBankSidebar);
document.getElementById('bankSidebarOverlay')?.addEventListener('click', window.closeBankSidebar);
document.getElementById('btnCloseBankSidebar')?.addEventListener('click', window.closeBankSidebar);

// 7. 攔截 switchMode (加入啟動時初始化題庫功能)
const originalSwitchMode = window.switchMode;
window.switchMode = function(mode, isForce = false) {
    if (mode === 'bank') {
        initBankCenter();
    }
    if (typeof originalSwitchMode === 'function') originalSwitchMode(mode, isForce);
};
...
window.viewPresetBank = function(index) {
    ...
    editor.value = preset.data;
    sheetTabs[activeSheetIndex].name = preset.name;
