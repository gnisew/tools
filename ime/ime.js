...
handleInput(e) {
    ...
    if (currentVal.length > this.lastInputValue.length) {
        ...
        // 修正行動版無法用空白鍵、數字鍵、w鍵的問題
        const currentToneMode = this.getCurrentToneMode();

        // 1. 處理空白鍵選第一個候選字
        if (diff === ' ' && this.compositionBuffer && this.allCandidates.length > 0) {
            this.selectCandidate(this.highlightedIndex);

            if (target.isContentEditable) { target.textContent = this.lastInputValue; } 
            else { target.value = this.lastInputValue; }
            this.lastInputValue = target.isContentEditable ? target.textContent : target.value;
            return;
        }
        
        // 2. 處理字母聲調模式下的數字鍵選字
        if (currentToneMode === 'alphabetic' && diff.match(/^[1-9]$/) && this.compositionBuffer && this.allCandidates.length > 0) {
        ...
        // 3. 處理 'w' 鍵轉換拼音
        if (diff.toLowerCase() === 'w' && this.compositionBuffer) {
