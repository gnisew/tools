...
imeInit(userConfig = {}) {
    ...
    // 特別處理 URL 參數中 outputEnabled 對 toolbarButtons 的影響
    if (typeof userConfig.outputEnabled === 'boolean') {
        userConfig.toolbarButtons = { ...fromStorage.toolbarButtons, outputModeToggle: userConfig.outputEnabled };
    }
    ...
    const finalSettings = {
        ...
        ...fromStorage,
        ...userConfig,
        // 確保 toolbarButtons 物件也被正確合併
        toolbarButtons: {
            ...defaults.toolbarButtons,
            ...fromStorage.toolbarButtons,
            ...userConfig.toolbarButtons
        }
    };
    ...
    // 步驟 5：如果設定來自 URL，則將「最終設定」回存到 LocalStorage
    const urlParamsProvided = Object.keys(userConfig).length > 0;
    if (urlParamsProvided) {
        console.log("Saving final settings from URL to localStorage:", finalSettings);
    ...
    // --- 後續的初始化流程 ---
    // (注意：這裡不再需要呼叫 imeLoad...Settings 等函式，因為設定已在上面處理完畢)
    ...
    // 更新設定視窗內的勾選狀態，使其與 finalSettings 同步
    if (this.settingsModal) {
        // 更新 Toolbar 按鈕顯示的 Checkbox
    ...
    console.log("WebIME initialized with final settings:", finalSettings);
},
