...
handleInput(e) {
    ...
    // 偵測到非 ASCII 輸入 (例如：選字、自動校正、表情符號)
    // 或偵測到刪除行為，且輸入法內還有編碼時，清空編碼。
    // 這是為了防止輸入框的內容與輸入法內部的狀態不同步。
    const lastChar = currentVal.slice(-1);
    const isAscii = lastChar.charCodeAt(0) <= 127;

    if ((!isAscii || currentVal.length < this.lastInputValue.length) && this.compositionBuffer) {
        this.compositionBuffer = '';
        this.compositionCursorPos = 0;
        this.updateCandidates();
    }
...
handleKeyDown(e) {
    ...
    // 【核心修改】
    // 移除了原有的 isMobile 判斷，讓手機和電腦共用相同的按鍵處理邏輯
    // 舊的判斷式為：if (this.isMobile && e.key && e.key.length === 1 && /[a-zA-Z0-9\s]/.test(e.key)) { return; }
    ...
    if (e.key.length === 1 && !reverseKeyMap[e.key]) { // 移除了 !this.isMobile 條件
        e.preventDefault();
        let character = e.key;
        const currentToneMode = this.getCurrentToneMode();
