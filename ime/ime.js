...
init(userConfig = {}) {
    ...
    this.boundUpdateStateAndReposition = this.updateStateAndReposition.bind(this);
...
activate(element) {
    ...
    // 【核心修改】
    // 強制重設狀態，對抗 Chrome 的狀態還原機制。
    // 透過延遲極短的時間來設定游標，可以覆蓋掉瀏覽器可能稍後執行的還原動作。
    setTimeout(() => {
        if (this.activeElement) {
            let len = 0;
            if (this.activeElement.isContentEditable) {
                // 對於 contenteditable div，需要更複雜的方式來移動游標到末端
                len = this.activeElement.textContent.length;
                const range = document.createRange();
                const sel = window.getSelection();
                if (this.activeElement.childNodes.length > 0) {
                     range.setStart(this.activeElement.childNodes[this.activeElement.childNodes.length - 1], this.activeElement.childNodes[this.activeElement.childNodes.length - 1].textContent.length);
                } else {
                    range.setStart(this.activeElement, 0);
                }
                range.collapse(true);
                sel.removeAllRanges();
                sel.addRange(range);
            } else {
                // 對於 input 和 textarea，可以直接設定
                len = this.activeElement.value.length;
                this.activeElement.setSelectionRange(len, len);
            }
            
            // 在強制設定游標後，再同步一次最新的狀態
            this.lastInputValue = this.activeElement.isContentEditable ? this.activeElement.textContent : this.activeElement.value;
        }
    }, 0); // 使用 setTimeout 0ms 延遲執行，確保在瀏覽器自己的事件處理之後
    ...
    this.activeElement.removeEventListener('click', this.boundUpdateStateAndReposition);
    this.activeElement.removeEventListener('keyup', this.boundUpdateStateAndReposition);
    this.activeElement.removeEventListener('mouseup', this.boundUpdateStateAndReposition);
    ...
    this.activeElement.addEventListener('click', this.boundUpdateStateAndReposition);
    this.activeElement.addEventListener('keyup', this.boundUpdateStateAndReposition);
    this.activeElement.addEventListener('mouseup', this.boundUpdateStateAndReposition);
...
updateStateAndReposition() {
    if (!this.activeElement) return;

    // 【關鍵修改】在重定位之前，先更新 lastInputValue
    // 這確保了不論游標點到哪裡，IME都知道當前的完整文字內容
    this.lastInputValue = this.activeElement.isContentEditable ? 
...
handleInput(e) {
    ...
    if (target.isContentEditable) {
        ...
        if (selection && selection.rangeCount > 0) {
            cursorPos = selection.getRangeAt(0).endOffset;
        }
    } else {
        cursorPos = target.selectionStart;
    }

    const isEditingInMiddle = cursorPos < currentVal.length;
    const isDeleting = currentVal.length < this.lastInputValue.length;
    const isUnusualChange = (currentVal.length - this.lastInputValue.length) !== 1 && !isDeleting;

    if (isEditingInMiddle || isDeleting || isUnusualChange) {
        // 重設輸入法狀態
        if (this.compositionBuffer) {
            this.compositionBuffer = '';
            this.compositionCursorPos = 0;
            this.updateCandidates(); // 會隱藏候選字窗
        }
        // 同步最新狀態後，結束函式
    ...
    // --- 如果程式能執行到這裡，代表使用者正在文字末端追加輸入 ---
    const diff = currentVal.substring(this.lastInputValue.length);

    // (此處保留了原版的邏輯，因為它們是處理正常組字的核心)
    const isAsciiRegex = /^[ -~]*$/;
    ...
    // 處理空白鍵選字
    if (diff === ' ') {
        if (!this.compositionBuffer && !this.isPredictionState) {
            this.lastInputValue = currentVal;
            return;
        }
        // 還原輸入框內容，準備送出文字
        if (target.isContentEditable) {
            target.textContent = this.lastInputValue;
        } else {
    ...
    // 這是實現「打 a b c，輸入框內不出現 a b c」效果的關鍵
    ...
    if (target.isContentEditable) {
        ...
        if (target.childNodes.length > 0) {
            range.setStart(target.childNodes[target.childNodes.length - 1], restoreVal.length);
        } else {
        ...
        target.setSelectionRange(restoreVal.length, restoreVal.length);
    }
    
    let inputChar = diff;
    const langProps = imeLanguageProperties[this.currentMode] || {};
    const isNumericToneMode = this.getCurrentToneMode() === 'numeric' && langProps.numericToneMap;
    if (isNumericToneMode && inputChar.match(/^[0-9]$/)) {
        inputChar = langProps.numericToneMap[inputChar] || inputChar;
    }
    
    this.compositionBuffer += inputChar;
