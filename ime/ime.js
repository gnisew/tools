...
(function() {
    ...
    config: {
        ...
        storagePrefix: 'webime_4_',   // 用於 localStorage 的前綴
...
imeSavePredictionMappingSettings() {
    ...
    this.imeShowToast('聯想詞來源已儲存');
},


/**
 * 初始化函式（修正版）
 * 徹底修正設定載入與儲存順序，確保 URL 參數有最高優先級，並能正確回存。
...
imeInit(userConfig = {}) {
    ...
    // 步驟 2：從 LocalStorage 載入所有「已儲存的設定」
    const fromStorage = {
        defaultMode: localStorage.getItem(this.config.storagePrefix + 'mode'),
        longPhrase: localStorage.getItem(this.config.storagePrefix + 'longPhrase') !== null ? localStorage.getItem(this.config.storagePrefix + 'longPhrase') === 'true' : undefined,
        initialFullWidth: localStorage.getItem(this.config.storagePrefix + 'fullWidth') !== null ? localStorage.getItem(this.config.storagePrefix + 'fullWidth') === 'true' : undefined,
        enablePrediction: localStorage.getItem(this.config.storagePrefix + 'prediction') !== null ? localStorage.getItem(this.config.storagePrefix + 'prediction') === 'true' : undefined,
        outputMode: localStorage.getItem(this.config.storagePrefix + 'outputMode'),
        outputEnabled: localStorage.getItem(this.config.storagePrefix + 'outputModeActive') !== null ? localStorage.getItem(this.config.storagePrefix + 'outputModeActive') === 'true' : undefined,
        toneModes: JSON.parse(localStorage.getItem(this.config.storagePrefix + 'toneModes') || 'null'),
        toolbarButtons: JSON.parse(localStorage.getItem(this.config.storagePrefix + 'toolbarSettings') || 'null'),
        userKeyMap: JSON.parse(localStorage.getItem(this.config.storagePrefix + 'userKeyMap') || 'null'),
        querySettings: JSON.parse(localStorage.getItem(this.config.storagePrefix + 'querySettings') || 'null'),
    };

    // 清理 fromStorage 中為 null 或 undefined 的值，避免它們覆蓋掉預設值
    Object.keys(fromStorage).forEach(key => (fromStorage[key] === null || fromStorage[key] === undefined) && delete fromStorage[key]);

    // 步驟 2.5：從 URL 參數推導應顯示的工具列按鈕
    const urlToolbarSettings = {};
    const urlParamsProvided = Object.keys(userConfig).length > 0;
    if (urlParamsProvided) {
        // 共享的設定意味著該功能的按鈕應該是可見的，這樣使用者才能操作
        if (typeof userConfig.outputEnabled === 'boolean' || userConfig.outputMode) {
            urlToolbarSettings.outputModeToggle = true;
        }
        if (userConfig.initialToneMode) {
            urlToolbarSettings.toneMode = true;
        }
        if (typeof userConfig.longPhrase === 'boolean') {
            urlToolbarSettings.longPhrase = true;
        }
        // 「全形/半形」按鈕預設為總是可見，所以不特別從 URL 推斷
        urlToolbarSettings.punctuation = true;
    }

    // 步驟 3：合併設定，確立最終值 (優先級: URL > LocalStorage > 預設)
    const finalSettings = {
        ...defaults,
        ...fromStorage,
        ...userConfig,
        // 確保 toolbarButtons 物件也被正確合併，URL 推導出的設定有最高優先級
        toolbarButtons: {
            ...defaults.toolbarButtons,
            ...(fromStorage.toolbarButtons || {}),
            ...urlToolbarSettings
        }
    };

    // 步驟 4：使用「最終設定」來一次性設定好輸入法的所有狀態
    this.currentMode = dictionaries[finalSettings.defaultMode] ? finalSettings.defaultMode : defaults.defaultMode;
    this.isLongPhraseEnabled = finalSettings.longPhrase;
    this.isFullWidthMode = finalSettings.initialFullWidth;
    this.config.enablePrediction = finalSettings.enablePrediction;
    this.config.outputMode = ['pinyin', 'word_pinyin', 'word_pinyin2'].includes(finalSettings.outputMode) ? finalSettings.outputMode : 'word';
    this.config.outputModeActive = finalSettings.outputEnabled;
    this.config.toolbarButtons = finalSettings.toolbarButtons;
    this.config.userKeyMap = finalSettings.userKeyMap;
    this.config.querySettings = finalSettings.querySettings;
    
    // 處理聲調模式
    this.toneModes = finalSettings.toneModes || {};
    if (finalSettings.initialToneMode) {
        this.toneModes[this.currentMode] = finalSettings.initialToneMode;
    }

    // 步驟 5：如果設定來自 URL，則將「最終設定」完整回存到 LocalStorage
    if (urlParamsProvided) {
        console.log("從 URL 來的設定，正在儲存最終結果到 localStorage:", finalSettings);
        localStorage.setItem(this.config.storagePrefix + 'mode', this.currentMode);
        localStorage.setItem(this.config.storagePrefix + 'longPhrase', String(this.isLongPhraseEnabled));
        localStorage.setItem(this.config.storagePrefix + 'fullWidth', String(this.isFullWidthMode));
        localStorage.setItem(this.config.storagePrefix + 'prediction', String(this.config.enablePrediction));
        localStorage.setItem(this.config.storagePrefix + 'outputMode', this.config.outputMode);
        localStorage.setItem(this.config.storagePrefix + 'outputModeActive', String(this.config.outputModeActive));
        localStorage.setItem(this.config.storagePrefix + 'toneModes', JSON.stringify(this.toneModes));
        // 儲存合併後最正確的 toolbarButtons 狀態
    ...
    if (this.settingsModal) {
        for (const key in this.config.toolbarButtons) {
            const checkbox = this.settingsModal.querySelector(`#toggle-btn-${key}`);
            if (checkbox) checkbox.checked = this.config.toolbarButtons[key];
        }
        const outputSelect = this.settingsModal.querySelector('#output-mode-select');
        if (outputSelect) {
            outputSelect.value = this.config.outputMode;
            outputSelect.style.display = this.config.toolbarButtons.outputModeToggle ? 'inline-block' : 'none';
        }
    }
    ...
    this.imeUpdateToolbarButtonsVisibility();
    this.imeUpdateToneModeButtonUI();
    this.imeUpdateOutputModeButtonUI();
    this.imeUpdatePunctuationButtonUI();
    this.imeAttachEventListeners();
    window.addEventListener('resize', this.boundEnsureInBounds);

    this.boundGlobalKeyDownHandler = (e) => {
        if (e.ctrlKey && e.key === '/') { e.preventDefault(); this.imeToggleIsEnabled(); }
        else if (e.ctrlKey && e.key === '\\') { e.preventDefault(); this.imeToggleOutputMode(); }
    };
    document.addEventListener('keydown', this.boundGlobalKeyDownHandler);
    
    this.isInitialized = true;
    console.log("WebIME 初始化完成，最終設定:", finalSettings);
},
...
imeCreateUI() {
    ...
    this.toneModeToggleBtn.addEventListener('click', () => this.imeToggleToneMode());
    ...
    this.longPhraseToggleBtn.addEventListener('click', () => this.imeToggleLongPhraseMode());
    ...
    this.punctuationModeToggleBtn.addEventListener('click', () => this.imeTogglePunctuationMode());
    ...
    this.outputModeToggleBtn.addEventListener('click', () => this.imeToggleOutputMode());
...
imeCreateExternalToolbar() {
    ...
    createAndRegister(this.toneModeToggleBtn, () => this.imeToggleToneMode(), this.allToneModeButtons);
    createAndRegister(this.longPhraseToggleBtn, () => this.imeToggleLongPhraseMode(), this.allLongPhraseButtons);
    createAndRegister(this.punctuationModeToggleBtn, () => this.imeTogglePunctuationMode(), this.allPunctuationButtons);
    createAndRegister(this.outputModeToggleBtn, () => this.imeToggleOutputMode(), this.allOutputModeButtons);
...
imeCreateSettingsModal() {
    ...
    predictionCheckbox.onchange = () => this.imeSavePredictionSettings();
    ...
    for (const key in buttonOptions) {
        ...
        checkbox.onchange = () => this.imeSaveToolbarSettings();
        label.appendChild(checkbox);
        label.appendChild(document.createTextNode(` ${labelText}`));
        buttonsContainer.appendChild(label);
    }

    const imeOutputModeLabel = document.createElement('label');
    imeOutputModeLabel.id = 'web-ime-output-mode-setting-row';
    imeOutputModeLabel.className = 'keymap-setting-row';
    ...
    imeOutputModeLabel.appendChild(mainCheckbox);
    imeOutputModeLabel.appendChild(document.createTextNode(' 輸出字音'));
    ...
    // 當下拉選單變更時，儲存設定
    outputModeSelect.addEventListener('change', () => this.imeSaveOutputModeSettings());
    
    imeOutputModeLabel.appendChild(outputModeSelect);

    // 4. 綁定主開關的事件
    mainCheckbox.addEventListener('change', () => {
        this.imeSaveToolbarSettings(); // 儲存主開關狀態
        // 連動顯示/隱藏下拉選單
        outputModeSelect.style.display = mainCheckbox.checked ? 'inline-block' : 'none';
    });
    
    buttonsContainer.appendChild(imeOutputModeLabel);
    ...
    keyMapInputs.forEach(input => {
        ...
        input.addEventListener('blur', () => {
            input.value = this.imeGetKeyDisplayName(input.dataset.key || '');
        });
    ...
    shareButton.addEventListener('click', () => {
        ...
        const settingsCode = [
            ...
            this.imeGetCurrentToneMode() === 'alphabetic' ? '1' : '0',
        ...
        navigator.clipboard.writeText(shareableUrl).then(() => {
            ...
            this.imeShowToast('複製失敗，您的瀏覽器可能不支援');
        });
    });
 ...
 */
imeUpdateToolbarButtonsVisibility() {
...
imeHandleInput(e) {
    ...
    if (currentVal.length > this.lastInputValue.length) {
        ...
        if (diff === ' ') {
            ...
            if (hasCandidates) {
                this.imeSelectCandidate(this.highlightedIndex); // 有候選字，選字
            } else if (hasBuffer) {
                this.compositionBuffer = ''; // 有編碼但無候選字，清空
                this.compositionCursorPos = 0;
                this.imeUpdateCandidates();
            } else {
                this.imeCommitText(' '); // 無編碼，直接上屏一個空格
            }
            return;
        }
        ...
        // 【主要修改點】
        // 在判斷 isWTransform 之前，先檢查當前語言是否允許拼音轉換
        const langProps = imeLanguageProperties[this.currentMode] || {};
...
imeHandleKeyDown(e) {
    ...
    // 【核心修改】
    // 在正則表達式中加入了「空白鍵(\s)」，讓 imeHandleKeyDown 在手機上忽略空白鍵與數字鍵，
    // 將其完全交給 imeHandleInput 處理，避免事件衝突。
    ...
    if (action) {
        switch (action) {
            case 'imeSelectCandidate':
                ...
                // 因為我們已經處理完畢，使用 return 來結束整個 imeHandleKeyDown 函數的執行，
                // 避免後續其他程式碼可能造成的干擾。
                return;
    ...
    if (!this.isMobile && e.key.length === 1 && !reverseKeyMap[e.key]) {
        ...
        // 將原本的 || 判斷式，改為更嚴謹的 in 判斷式。
        // 這樣即使 numericToneMap[character] 的值是空字串 ('')，也能被正確處理。
...
imeUpdateToneModeButtonUI() {
    const langProps = imeLanguageProperties[this.currentMode] || {};
    // 取得當前的聲調模式設定
    const currentSetting = this.imeGetCurrentToneMode();
    // 決定按鈕上要顯示的文字
    const modeText = currentSetting === 'numeric' ? '調ˇ' : 'źv̌s̀';

    // 遍歷所有註冊的「字母/數字」按鈕
    this.allToneModeButtons.forEach(btn => {
        // 直接更新所有按鈕的文字內容，不再判斷或修改 display 樣式
        if (btn) {
...
imeToggleOutputMode() {
    ...
    this.imeShowToast(`字音輸出已${statusText}`);
},

imeUpdateOutputModeButtonUI() {
    // 【修改】遍歷所有相關按鈕並更新
    this.allOutputModeButtons.forEach(btn => {
        if (!btn) return;
        const isEnabled = this.config.outputModeActive;
        const icon = isEnabled ? 'translate' : 'translate';
...
imeSelectCandidate(indexOnPage) {
	...
	if (this.compositionBuffer.length > 0) {
		...
		if (shouldShowPredictions) {
			const predictions = this.imeFindPredictionCandidates(selectedWord);
