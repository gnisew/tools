...
/**
 * 處理游標移動或文字選取變化的事件。
 * 這是解決行動裝置上游標移動後輸入異常的關鍵。
 */
handleSelectionChange() {
    // 如果沒有作用中的元素，或在非行動裝置上，則不處理
    if (!this.activeElement || !this.isMobile) {
        return;
    }
    ...
    // 當使用者移動游標時，如果當前有正在輸入的編碼 (compositionBuffer)，
    // 這代表使用者意圖放棄這次的輸入，去別的地方。
    // 因此我們需要重設輸入狀態，以避免下次輸入時發生邏輯錯誤。
    if (this.compositionBuffer) {
        this.compositionBuffer = '';
        this.compositionCursorPos = 0;
        this.updateCandidates(); // 清空並隱藏候選字容器
    }

    // 同步 lastInputValue，確保下一次的 handleInput 事件能正確運作
...
reposition() {
    // 使用 setTimeout 將整個定位邏輯延遲到下一個事件週期執行
    setTimeout(() => {
