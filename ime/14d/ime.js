...
handleKeyDown(e) {
    ...
    // 在行動版，讓英數和空白鍵完全交給 handleInput 處理
    ...
    if (action) {
        switch (action) {
            ...
            // 【核心修正處】
            case 'commitComposition': // 這個 action 對應到 'Enter' 鍵
                // 只有在「同時」滿足「有輸入碼」和「有候選字」這兩個條件時，
                // 我們才需要攔截 Enter 鍵來輸出文字。
                ...
                // 如果不滿足以上條件 (例如只有輸入碼但無候選字，或是根本沒輸入碼)，
                // 程式會直接跳出這個 if，執行下面的 break。
                // 這樣就不會執行 preventDefault，Enter 鍵就能正常換行。
                // 我們需要在這種情況下，手動清掉無效的輸入碼。
                if (hasComposition && !hasCandidates) {
                    this.compositionBuffer = '';
                    this.compositionCursorPos = 0;
                    this.updateCandidates();
