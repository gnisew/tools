...
attachEventListeners() {
	...
	this.handleFocusOut = (e) => {
		// 如果點擊的是輸入法本身，則立即返回，不進行任何操作
		if (this.isClickingInside) {
			this.isClickingInside = false; // 重設旗標
			// 並且確保焦點仍在輸入框上
		...
		// --- 【核心修改】 ---
		// 使用 setTimeout 將失焦的判斷延遲到下一個事件循環
		// 這可以有效過濾掉行動版 Chrome 在同一個輸入框內移動游標時觸發的假性 focusout 事件
		setTimeout(() => {
			// 在延遲後，再次檢查當前擁有焦點的元素 (document.activeElement)
			// 是否還是我們輸入法正在作用的元素 (this.activeElement)
			// 如果不是，代表焦點真的離開了，此時才進行停用和隱藏
