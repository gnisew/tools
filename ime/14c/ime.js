    ...
    if (action) {
        switch (action) {
            ...
            case 'commitComposition': // 這個 action 對應到 'Enter' 鍵
                 if (hasComposition) { // 只有在輸入區有內容時才需要判斷
                    ...
                    if (!hasCandidates) {
                        ...
                        this.compositionBuffer = '';
                        this.compositionCursorPos = 0;
                        this.updateCandidates(); // 更新UI，隱藏輸入區
                        
                        // 2. 直接 return，不呼叫 e.preventDefault()
                        //    這樣瀏覽器就會執行 Enter 鍵的預設行為 -> 換行
                        return; 
                    }
                    
                    // 如果程式能執行到這裡，表示有輸入內容且有候選字
                    // 維持原本的邏輯：輸出輸入區的文字
                    e.preventDefault();
                    this.commitText(this.compositionBuffer);
                    this.compositionBuffer = '';
