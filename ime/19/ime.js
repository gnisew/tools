...
imeInit(userConfig = {}) {
    ...
    // 【主要修改點】
    // 將所有 .bind(this) 的操作提前到函式的最開頭。
    // 這樣可以確保在呼叫任何其他函式（如 imeCreateUI）之前，
    // 所有需要用到的綁定函式都已經準備就緒。
    ...
    this.boundGlobalKeyDownHandler = (e) => {
        ...
        switch (e.key) {
            ...
            case '\\': { // 使用大括號建立獨立作用域
                ...
                this._syncFeatureSettings(newSettings); // 套用並儲存設定
			...
			// Ctrl + Shift + , ：切換字音輸出
			case '<': {
				if (!e.shiftKey) return; // 必須同時按 Shift
				e.preventDefault();
				const newSettings = { ...this.config.features };
...
imeCreateUI() {
    ...
    // 移除了 toneModeToggleBtn, longPhraseToggleBtn, punctuationModeToggleBtn, outputModeToggleBtn 的建立與附加程式碼
...
imeCreateExternalToolbar() {
    ...
    // 移除了複製 toneMode, longPhrase, punctuation, outputMode 按鈕的相關程式碼
...
imeCreateSettingsModal() {
    ...
    const features = [
        ...
        { id: 'prediction', text: '聯想詞', default: false },
        { id: 'numericTone', text: '數字鍵調 (未啟用為 字母鍵調zvs)', default: true },
        { id: 'longPhrase', text: '長詞連打', default: false },
        { id: 'fullWidthPunctuation', text: '全形標點', default: true },
        { id: 'outputEnabled', text: '字音輸出', default: false, hasSelect: true }
    ];
    ...
    features.forEach(feature => {
        ...
        if (feature.hasSelect) {
            ...
            // --- 修改下拉選單選項 ---
            const options = [
                { value: 'pinyin_mode', text: '拼音模式' }, // 新增此選項並設為第一個
                { value: 'pinyin', text: '僅拼音' },
    ...
    shareButton.addEventListener('click', () => {
        ...
        if (this.config.features.outputEnabled) {
            ...
            switch (this.config.features.outputMode) {
                case 'pinyin': outputModeCode = '1'; break;
                case 'word_pinyin': outputModeCode = '2'; break;
                case 'word_pinyin2': outputModeCode = '3'; break;
                case 'pinyin_mode': outputModeCode = '4'; break; // 新增 pinyin_mode 的代碼
            }
        }
        const settingsCode = [
...
/**
 * (全新輔助函式) 套用指定的 features 物件到輸入法狀態、
...
imeHandleInput(e) {
    ...
    if (currentVal.length > this.lastInputValue.length) {
        ...
        // 同樣地，在行動裝置的 input 事件中也加入此判斷。
        if (this.isPredictionState && this.compositionBuffer === '' && this.config.outputMode === 'pinyin_mode') {
            this.imeCommitText(' ');
        }
...
imeHandleKeyDown(e) {
    ...
    if (!this.isMobile && e.key.length === 1 && !reverseKeyMap[e.key]) {
        ...
        // --- 【*** 本次修改核心 ***】 ---
        // 如果目前正在顯示聯想詞 (isPredictionState 為 true)，
        // 且使用者開始輸入新的編碼 (compositionBuffer 為空)，
        // 代表這是一個新詞的開始，需要在前面補上空格。
        if (this.isPredictionState && this.compositionBuffer === '' && this.config.outputMode === 'pinyin_mode') {
            this.imeCommitText(' ');
        }
        // --- 【*** 修改結束 ***】 ---
...
/**
 * 尋找符合前綴的候選字。
 * 在比對無聲調的字典鍵時，會先將使用者的輸入碼也轉換為無聲調版本，
 */
imeFindSimplePrefixCandidates(buffer) {
    ...
    const normalizedBuffer = this.imeNormalizeCompositionBuffer(buffer, this.currentMode);
    // --- 【核心修正】 ---
    // 在這裡也將使用者的輸入碼簡化(移除聲調)，以便和簡化後的字典鍵進行比對。
    const simplifiedBuffer = this.imeSimplifyKey(normalizedBuffer, this.currentMode);
    ...
    const candidates = [];
    for (const entry of relevantEntries) {
        // 檢查 1: 比對原始拼音(帶聲調)。這讓使用者輸入完整拼音時能精準匹配。
        if (entry.originalKey.replace(/[\s-]+/g, '').startsWith(normalizedBuffer)) {
             candidates.push(...entry.values);
        }
        // 檢查 2: 比對簡化後的拼音(無聲調)。這讓使用者即使不輸入聲調也能找到詞。
        else if (entry.simplifiedKey.startsWith(simplifiedBuffer)) {
            candidates.push(...entry.values);
        }
    }
...
/**
 * [新版本 - 已整合拼音模式的純拼音聯想詞，並為聯想詞自動加上前置空格]
 * 處理使用者選擇候選字的邏輯。
 */
imeSelectCandidate(indexOnPage) {
    ...
    // 【*** 本次修改核心 ***】
    // 如果正處於聯想詞狀態 (代表這不是輸入的第一個詞)，
    // 並且是「拼音模式」或「僅拼音輸出」模式，
    // 則在送出的聯想詞前自動加上一個空格。
    if (this.isPredictionState && (this.config.outputMode === 'pinyin_mode' || (this.config.outputModeActive && this.config.outputMode === 'pinyin'))) {
