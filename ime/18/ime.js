...
imeLoadPosition() {
    ...
    if (pinnedTop && pinnedLeft) {
        ...
        this.toolbarContainer.style.right = 'auto'; // 清除 right 定位
...
imeSelectCandidate(indexOnPage) {
    ...
    // 在送出文字前，檢查是否處於聯想詞狀態，且輸出模式為純拼音
    // 如果是，則在輸出的拼音前加上一個空格，避免拼音連在一起
    if (this.isPredictionState && this.config.outputModeActive && this.config.outputMode === 'pinyin') {
