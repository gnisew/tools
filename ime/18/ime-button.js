...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (params.has('ime')) {
        ...
        if (parts.length === 3) {
            ...
            // 根據新的設定碼格式解析
            if (settingsCode.length >= 1) features.prediction = (settingsCode[0] === '1');
            if (settingsCode.length >= 2) features.numericTone = (settingsCode[1] === '1');
            if (settingsCode.length >= 3) features.longPhrase = (settingsCode[2] === '1');
            if (settingsCode.length >= 4) features.fullWidthPunctuation = (settingsCode[3] === '1');
            
            if (settingsCode.length >= 5) {
                features.outputEnabled = (settingsCode[4] === '1');
				if (settingsCode.length >= 6) {
					switch(settingsCode[5]) {
						case '1': features.outputMode = 'pinyin'; break;
						case '2': features.outputMode = 'word_pinyin'; break;
						case '3': features.outputMode = 'word_pinyin2'; break;
					}
				}
            }
        }
