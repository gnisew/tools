document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. 全域變數與 DOM 元素
    // ==========================================
    const STORAGE_KEY = 'wesing_music_data_v30';
    ...
    // ==========================================
    // 2. Data Constants (Mapping & Keys)
    // ==========================================
    
    // Mapping Data for Fonts
    const mappingData = [
    ...
    async function loadInstrument(instName) {
        ...
        try {
            const inst = await Soundfont.instrument(audioCtx, instName);
    ...
    function playTone(freq, startTime, duration, instVal) {
        ...
        if (instDef.type === 'soundfont') {
            if (activeSoundfontInst) {
                ...
                try {
                    const node = activeSoundfontInst.play(midi, startTime, { duration: duration });
    ...
    function transposeText(direction) {
        ...
        for(let i=0; i<parts.length; i++) {
            ...
            if(numMatch) {
                ...
                // Generate new token (Symbol + Space + Note)
                if(newAcc === 1) newParts.push("#"); // No extra space in string
                if(newAcc === -1) newParts.push("b");
                
                // Add explicit space after accidental if one exists
    ...
    function renderInstrumentList() {
        ...
        instruments.forEach(inst => {
            ...
            div.innerHTML = `${inst.name} <span class="sub-label">${inst.type === 'soundfont' ? '需下載' : '內建'}</span> <span class="inst-check">✓</span>`;
            div.onclick = () => {
