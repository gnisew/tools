...
function renderStory() {
    ...
    // 這裡將 padding 縮小 (p-4 -> p-2)，並強制 flex-row
    controls.innerHTML = `
        <div class="bg-white p-2 rounded-xl shadow-sm border border-gray-100 mb-4">
            
            <div class="flex gap-2 p-0.5 bg-gray-100 rounded-lg mb-2">
                <button onclick="setStoryMode('read')" class="flex-1 py-1.5 rounded-md text-xs font-bold transition-all flex items-center justify-center gap-2 ${state.story.mode === 'read' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}">
                    <i class="far fa-eye"></i> 閱讀
                </button>
                <button onclick="setStoryMode('quiz')" class="flex-1 py-1.5 rounded-md text-xs font-bold transition-all flex items-center justify-center gap-2 ${state.story.mode === 'quiz' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}">
            ...
            <div class="flex justify-between items-center">
                ...
                <div class="flex items-center gap-1">
                    <button onclick="speak('${speakText}', 1)" class="${btnBase} ${normalBtnClass}">
                        <i class="fas ${isPlayingThis && currentRate === 1 ? 'fa-stop' : 'fa-volume-up'}"></i>
                    </button>
                    <button onclick="speak('${speakText}', 0.7)" class="${btnBase} ${slowBtnClass}">
                        <span class="text-[10px] scale-75 transform">🐢</span>
                    </button>
                </div>
            </div>
        </div>
