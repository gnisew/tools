...
const exp2 = {
    ...
    init() {
        ...
        // 新增：點擊日期 input 時嘗試強制開啟原生日期選擇器
        // (桌面版 Chrome/Edge 需要明確呼叫 showPicker() 才會跳出選單；
        //  若瀏覽器不支援此 API（例如部分平板/舊版 Safari），則以 try/catch 忽略錯誤，
        //  改由瀏覽器對真實 input 點擊的原生行為開啟選擇器)
        this.el.date.addEventListener('click', () => {
            try { this.el.date.showPicker(); } catch (e) { /* 不支援時忽略，走原生點擊行為 */ }
        });
