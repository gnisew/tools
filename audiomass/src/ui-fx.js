	...
	function PKUI_FX ( app ) {
		...
		app.listenFor ('RequestFXUI_Gain', function () {
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row" style="border:none"><label>' + t('gain.label') + '</label>' +
		...
		app.listenFor ('RequestActionFXUI_Rate', function () {
			...
			var x = new PKAudioFXModal({
			  ...
			  title: t('speed.title'),
				presets:[
					{name: t('speed.a_lot_slower'),val:0.65},
					{name: t('speed.slightly_slower'),val:0.85},
					{name: t('speed.slightly_faster'),val:1.15},
					{name: t('speed.blazing_fast'),val:1.4}
				],
				custom_pres:custom_presets.Get (filter_id),
			ondestroy: function ( q ) {
				app.ui.InteractionHandler.on = false;
			...
			preview: function ( q ) {
			  ...
			  body:'<div class="pk_row" style="border:none"><label>' + t('speed.label') + '</label>' +
		...
		app.listenFor ('RequestActionFXUI_Speed', function () {
			...
			if (!(app.engine && app.engine.wavesurfer.backend.buffer) && !(mt && mt.IsOn && mt.IsOn ()))
				return OneUp (t('uifx.load_first'), 1200);
			app.fireEvent ('RequestSelect', 1);
			...
			var x = new PKAudioFXModal({
			  ...
			  title: t('pitch.title'),
					presets:[
						{name:'-1/4',val:0.25},
						{name:'-1/2',val:0.5},
						{name: t('pitch.slower'),val:0.85},
						{name: t('pitch.faster'),val:1.1},
						{name:'+1/4',val:1.25},
						{name:'+1/2',val:1.5},
						{name: t('pitch.doppler'),val:'profile:0:1.35|0.5:0.72|1:1.35'},
						{name: t('pitch.accelerate'),val:'profile:0:0.70|0.55:1.00|1:1.65'},
						{name: t('pitch.engine_rev'),val:'profile:0:0.82|0.25:1.35|0.58:1.08|1:1.55'},
						{name: t('pitch.car_approaching'),val:'profile:0:0.72|0.3:0.88|0.65:1.18|1:1.55'},
						{name: t('pitch.car_driving_away'),val:'profile:0:1.55|0.35:1.18|0.7:0.88|1:0.72'}
					],
				custom_pres:custom_presets.Get (filter_id),
				ondestroy: function ( q ) {
			  ...
			  body:'<div class="pk_row pk_mm" style="border:none;display:none">'+
				...
				'<input type="checkbox" class="pk_check pk_c_mm" id="xmm" name="makeMono">'+
					...
					'<div class="pk_row pk_ms" style="border:none;display:none">'+
						...
						'<label for="xms">' + t('flip.make_stereo') + '</label></div>' +
					'</div>',
			  setup:function( q ) {
			  	  var main = null;
		...
		app.listenFor ('RequestFXUI_Silence', function () {
			var x = new PKSimpleModal({
			  ...
			  body:'<div class="pk_row"><input type="radio" class="pk_check" id="ifeq" name="rdslnc" value="beginning">'+
				'<label  for="ifeq">' + t('silence.at_beginning') + '</label><br/>' +
				'<input type="radio" class="pk_check"  id="vgdja" name="rdslnc" checked value="cursor">'+
				'<label for="vgdja">' + t('silence.at_cursor') + ' (<span class="pkcdpk"></span>)</label></div>'+
				'<div class="pk_row"><label>' + t('silence.seconds') + '</label>'+
		...
		app.listenFor ('RequestActionFXUI_Compressor', function () {
			...
			ondestroy: function ( q ) {
				...
				presets:[
					{name: t('compressor.vocal_lead'),val:'-18,6,3,0.005,0.15,6'},
					{name: t('compressor.vocal_broadcast'),val:'-20,2,4,0.003,0.1,7.5'},
					{name: t('compressor.drum_bus'),val:'-10,3,2,0.03,0.3,2.5'},
					{name: t('compressor.snare_punch'),val:'-15,1,4,0.001,0.06,5.5'},
					{name: t('compressor.kick_tighten'),val:'-12,2,4,0.01,0.1,4.5'},
					{name: t('compressor.bass_even'),val:'-15,4,3,0.015,0.15,5'},
					{name: t('compressor.acoustic_smooth'),val:'-18,6,2.5,0.01,0.15,5.5'},
					{name: t('compressor.master_glue'),val:'-8,6,1.5,0.03,0.3,1.5'},
					{name: t('compressor.heavy_crush'),val:'-30,0,8,0.002,0.1,8'},
					{name: t('compressor.brickwall'),val:'-3,0,20,0.001,0.05,1.5'},
					{name: t('compressor.classic_3_1'),val:'-18,6,3,0.01,0.1,6'},
					{name: t('compressor.light_touch'),val:'-12,2,2,0.005,0.08,3'},
					{name: t('compressor.classic'),val:'-40,5,7,0.002,0.1,12'},
					{name: t('compressor.dashed_dist'),val:'-45,26,2.05,0.233,0.0,0'}
				],
				custom_pres:custom_presets.Get (filter_id),
			preview: function ( q ) {
				var inputs = q.el_body.getElementsByTagName('input');
		...
		app.listenFor ('RequestActionFXUI_Normalize', function () {
			...
			var x = new PKAudioFXModal({
			  ...
			  body:'<div class="pk_row pk_norm_modes"><label>' + t('normalize.mode') + '</label>'+
				...
				'<div class="pk_row"><label>' + t('normalize.target') + '</label>'+
				...
				'</optgroup>'+
				'</select></div>'+
				'<div class="pk_row pk_lufs_custom pk_lufs_rng" style="display:none"><label>' + t('normalize.custom_lufs') + '</label>'+
				'<input type="range" class="pk_horiz pk_w180 pk_lufs_custom_v" min="-40" max="-6" value="-14" step="0.1">'+
				'<span class="pk_val">-14.0</span></div>'+
				'<div class="pk_row pk_lufs_rng"><label>' + t('normalize.peak_ceiling') + '</label>'+
				'<input type="range" class="pk_horiz pk_w180 pk_lufs_ceil" min="-6" max="0" value="-1" step="0.1">'+
				'<span class="pk_val">-1.0 dBTP</span></div>'+
				'<div class="pk_row" style="border:none"><label>' + t('normalize.analysis') + '</label>'+
				'<span class="pk_lufs_out">' + t('normalize.measure_lufs') + '</span></div>'+
				'</div>',
			  setup:function( q ) {
				  ...
				  function resetLufs () {
					q._lufsReport = null;
					q._lufsNorm = null;
					out.innerHTML = t('normalize.measuring');
					syncApply ();
				  }
				  ...
				  q.AnalyzeLUFS = function () {
					...
					setTimeout (function () {
						app.fireEvent ('RequestActionFX_Loudness', function (report) {
							q._measuring = false;
							if (!report) {
								out.innerHTML = t('normalize.could_not_analyze');
								return ;
							}
		...
		app.listenFor ('RequestActionFXUI_Repair', function () {
			...
			if (mt && mt.IsOn && mt.IsOn ())
				return OneUp (t('uifx.repair_not_available'), 1400);
					...
					if (!is_new)
					{
							...
							btn_delete = {
									...
									callback: function( q ) {
										...
										OneUp (t('uifx.preset_deleted'), 1400);
							...
							btn_update = {
									...
									callback: function( q ) {
										...
										if (custom_obj)
										{
											...
											if (value.length > 0)
											{
												OneUp (t('uifx.preset_updated'), 1400);
											...
											{
												OneUp (t('uifx.name_too_short'), 1200);
											}
										}
										// -
									}
					  ...
					  buttons:[
						{
							...
							callback: function( q ) {
								...
								if (value.length > 0)
								{
									OneUp (t('uifx.preset_saved'), 1400);
		...
		app.listenFor ('RequestShowFreqAn', function ( url, args_arr ) {
			...
			if (!type)
			{
				var makePopup = function ( dat ) {
					...
					if (!wnd) {
						OneUp (t('uifx.allow_popups'), 3600, 'pk_r');
						return ;
					}
