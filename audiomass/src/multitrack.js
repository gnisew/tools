(function ( w, d, PKAE ) {
	...
	function PKMultitrack ( app ) {
		...
		function makeTrack ( name ) {
			return {
				id: 'mt' + (track_uid++),
				name: name || (t('mt.channel_n') + track_uid),
				mute: false,
				solo: false,
				vol: 1,
				pan: 0,
		...
		function ExportSession ( name ) {
			if (!tracks.length) {
				OneUp (t('mt.msg.nothing_save'), 1200);
				return false;
			}
			var st = cloneState ();
			...
			{
				OneUp (t('mt.msg.save_fail'), 1400);
				return false;
			}
			OneUp (t('mt.msg.saved'), 1000);
			return true;
		}
		...
		function LoadSessionBuffer ( buf, name ) {
			...
			try {
				...
				OneUp (t('mt.msg.loaded') + (name ? ': ' + name : ''), 1200);
			}
			catch (e) {
				OneUp (t('mt.msg.load_fail'), 1400);
			}
		...
		function buildHeader () {
			...
			var add = makeButton ('+', t('mt.add_channel'), false, 'pk_mt_add');

			label.textContent = t('mt.channels');
		btn_clear_mute = makeButton ('M', t('mt.clear_mute'), false, 'pk_mt_clear pk_mt_mute pk_inact');
			btn_clear_solo = makeButton ('S', t('mt.clear_solo'), false, 'pk_mt_clear pk_mt_solo pk_inact');
		...
		function attachToolbarButton () {
			...
			btn_toggle.innerHTML = t('mt.toggle');
		...
		btn_beat = makeButton (t('mt.beat'), t('mt.toggle_beat'), beat_on, 'pk_btn pk_mtbeat_btn');
			btn_snap = makeButton (t('mt.snap'), t('mt.snap_beat'), false, 'pk_btn pk_mtbeat_btn pk_mtbeat_snap');
			btn_sig = makeButton (beat_sig, t('mt.time_sig'), false, 'pk_btn pk_mtbeat_sig');
		...
		function renderTrack ( track, top, h ) {
			...
			input.onchange = function () {
				...
				track.name = val;
				input.value = val;
				pushState ( prev, t('mt.desc.rename_channel') );
			};

			var mute = makeButton ('M', t('mt.mute'), track.mute, 'pk_mt_mute');
			var solo = makeButton ('S', t('mt.solo'), track.solo, 'pk_mt_solo');
			var arm = makeButton ('R', t('mt.rec_trigger'), track.rec, 'pk_mt_rec');
			var del = makeButton ('x', t('mt.delete_channel'), false, 'pk_mt_del');
		...
		function setRecordArm ( track, value ) {
			...
			if (!changed) return ;
			selected_track = track.id;
			pushState ( prev, t('mt.desc.arm_channel') );
			render ();
		}
		...
		function removeTrack ( track ) {
			...
			cleanXfades ();
			pushState ( prev, t('mt.desc.remove_channel') );
		...
		function bindVolume ( knob, track ) {
			...
			function up () {
				...
				clearActiveDrag ( up );
				if (moved) pushState ( prev, t('mt.desc.volume_channel') );
			}
		}
		...
		function bindPan ( knob, track ) {
			...
			function up () {
				if (stop_drag) stop_drag ();
				clearActiveDrag ( up );
				if (moved) pushState ( prev, t('mt.desc.pan_channel') );
			}
		}
		...
		function startTrackResize ( e, track ) {
			...
			function up () {
				...
				if (moved) pushState ( prev, t('mt.desc.resize_channel') );
			}
		}
		...
		function startTrackReorder ( e, track ) {
			...
			function up () {
				...
				if (moved && trackIndex ( track.id ) !== start_i) {
					pushState ( prev, t('mt.desc.reorder_channel') );
					app.fireEvent ('DidUpdateMultitrack');
				}
			}
		}
		...
		function moveSelectedTrack ( diff ) {
			...
			tracks.splice (old, 1);
			tracks.splice (index, 0, track);
			pushState ( prev, diff < 0 ? t('mt.desc.move_channel_up') : t('mt.desc.move_channel_down') );
			render ();
		...
		function toggleXfade () {
			...
			if (!pairs.length) {
				OneUp (t('mt.msg.no_overlap'), 1200);
				return true;
			}
			...
			pushState ( prev, all_on ? t('mt.desc.remove_crossfade') : t('mt.desc.crossfade_clip') );
			queuePlayRefresh ( true );
			render ();
			OneUp (all_on ? t('mt.ctx.crossfade_off') : t('mt.ctx.crossfade'), 900);
			return true;
		}
		...
		function buildClipContext () {
			...
			mt_context.addOption (t('mt.ctx.open_editor'), function () {
				doContextClip ( loadClip );
			}, false);
			mt_context.addOption (t('mt.ctx.rename_clip'), function () {
				doContextClip ( renameSelectedClip );
			}, false);
			mt_context.addOption (t('mt.ctx.duplicate_clip'), function () {
				doContextClip ( duplicateSelectedClip );
			}, false);
			mt_context.addOption (t('mt.ctx.copy_clip'), function () {
				doContextClip ( copySelectedClip );
			}, false);
			mt_context.addOption (t('mt.ctx.split_here'), function () {
				doContextClip (function () { splitSelectedClip ( context_time ); });
			}, false);
			mt_context.addOption (t('mt.ctx.delete_clip'), function () {
				doContextClip ( deleteSelectedClip );
			}, false);
			mt_context.addOption (t('mt.ctx.crossfade'), function () {
				doContextClip (function ( clip ) {
					xfadeState ( clip ) && toggleXfade ();
				});
			}, false);
			mt_context.addOption (t('mt.ctx.fade_in'), function () {
				doContextClip (function () { applyFx ('FadeIn'); });
			}, false);
			mt_context.addOption (t('mt.ctx.fade_out'), function () {
			...
			mt_context.onOpen = function ( menu, div ) {
				...
				a[6].innerHTML = x === 2 ? t('mt.ctx.crossfade_off') : t('mt.ctx.crossfade_on');
				a[6].className = 'pk_ctx_action' + (x ? '' : ' pk_inact');
				Pause ();
			};

			mt_lane_context = app._deps.ContextMenu ( lanes );
			mt_lane_context.addOption (t('context.select_visible'), function () {
				app.fireEvent ('RequestRegionSet');
			}, false);
			mt_lane_context.addOption (t('mt.ctx.add_channel_here'), function () {
				addTrack ( context_index );
			}, false);
			mt_lane_context.addOption (t('context.reset_zoom'), function () {
				app.fireEvent ('RequestZoomUI', 0);
			}, false);
			mt_lane_context.addOption (t('mt.ctx.add_silence'), function () {
		...
		function renderRecPreview () {
			...
			if (!rec_el || !rec_el.parentNode) {
				...
				rec_el.innerHTML = '<canvas></canvas><span>' + t('mt.recording') + '</span>';
		...
		function bindClipDrag ( ce, clip ) {
			...
			function up ( e ) {
				...
				if (did_move && (
					...
					pushState ( prev, drag_mode > 2 ? t('mt.desc.fade_clip') : (drag_mode ? t('mt.desc.trim_clip') : t('mt.desc.move_clip')) );
					queuePlayRefresh ( true );
					render ();
					return ;
				}
		...
		function addFiles ( file_list, track_id, start ) {
			...
			function done () {
				...
				if (added && (multi || findTrack ( track_id ))) {
					applyZoom ( zoom );
					pushState ( prev, multi ? t('mt.desc.add_tracks') : t('mt.desc.add_clip') );
					queuePlayRefresh ( true );
					render ();
					restoreZoomScroll ( zoom );
					app.fireEvent ('DidUpdateMultitrack');
					OneUp (t('mt.msg.added') + ' ' + added + (multi ?
						' ' + t(added === 1 ? 'mt.msg.added_track' : 'mt.msg.added_tracks') :
						' ' + t(added === 1 ? 'mt.msg.added_clip' : 'mt.msg.added_clips')));
				}
				else {
					OneUp (missing_track ? t('mt.msg.channel_removed') : t('mt.msg.decode_fail'), 1200);
				}
			}
		}
		...
		function renameSelectedClip () {
			...
			var prev = cloneState ();
			clip.name = name;
			pushState ( prev, t('mt.desc.rename_clip') );
			render ();
			app.fireEvent ('DidUpdateMultitrack');
			return true;
		}
		...
		function duplicateSelectedClip () {
			...
			pushState ( prev, t('mt.desc.duplicate_clip') );
			queuePlayRefresh ( true );
			render ();
			app.fireEvent ('DidSelectClip', dup);
			app.fireEvent ('DidUpdateMultitrack');
			OneUp (t('mt.msg.duplicated'), 900);
			return true;
		}
		...
		function selectedFxClip () {
			var clip = findClip ( selected_clip );
			if (!clip) OneUp (t('mt.msg.select_first'), 1200);
			return clip;
		}
		...
		function fxLabel ( name ) {
			return {
				Gain: t('fx.label.gain'),
				FadeIn: t('fx.label.fade_in'),
				FadeOut: t('fx.label.fade_out'),
				HardLimit: t('fx.label.hard_limit'),
				ParametricEQ: t('fx.label.parametric_eq'),
				Compressor: t('fx.label.compressor'),
				Normalize: t('fx.label.normalize'),
				NormalizeRMS: t('fx.label.rms_normalize'),
				NormalizeLUFS: t('fx.label.lufs_normalize'),
				Delay: t('fx.label.delay'),
				Distortion: t('fx.label.distortion'),
				Reverb: t('fx.label.reverb'),
				Reverse: t('fx.label.reverse'),
				Invert: t('fx.label.invert'),
				Rate: t('fx.label.rate'),
				Speed: t('fx.label.speed')
			}[name] || name;
		}
		...
		function applyFx ( name, val ) {
			...
			function done ( rendered ) {
				...
				OneUp (name === 'NormalizeLUFS' && val && val.limited ?
					t('mt.msg.applied_lufs') + ' (fx)' :
					t('mt.msg.applied_fx') + ' ' + fxLabel ( name ) + ' (fx)');
			}
		...
		function pasteClip () {
			...
			pushState ( prev, t('mt.desc.paste_clip') );
			queuePlayRefresh ( true );
			render ();
			app.fireEvent ('DidSelectClip', clip);
			app.fireEvent ('DidUpdateMultitrack');
			OneUp (t('mt.msg.pasted'), 900);
			return true;
		}
		...
		function GetTempoBuffer ( selection_only ) {
			...
			OneUp (t('mt.msg.select_first'), 1200);
			return null;
		}
		...
		function addSilence ( offset, seconds ) {
			...
			clearRegion ();
			setMarkerTime ( clip.start );
			pushState ( prev, t('mt.desc.silence') );
			queuePlayRefresh ( true );
			render ();
			app.fireEvent ('DidUpdateMultitrack');
			app.fireEvent ('DidSelectClip', clip);
			OneUp (t('mt.msg.inserted_silence'));
			return true;
		}
		...
		function addURL ( url, name ) {
			fetchBlob ( url, function ( blob ) {
				...
				OneUp (t('mt.msg.load_audio_fail'), 1200);
			});
		}
		...
		function addMultiSampleBuffers ( prev, items, replace, zoom ) {
			...
			for (var n = 0; n < multi_sample_files.length; ++n) {
				if (items[n]) continue ;
				OneUp (t('mt.msg.load_samples_fail'), 1400);
				return ;
			}
			...
			setBeatBpm ( 136 );
			applyZoom ( zoom );
			pushState ( prev, t('mt.desc.load_sample') );
			queuePlayRefresh ( true );
			render ();
			restoreZoomScroll ( zoom );
			...
			app.fireEvent ('DidUpdateMultitrack');
			OneUp (added + ' ' + t('mt.msg.added_samples'));
		}
		...
		function loadClip ( clip ) {
			...
			OneUp (t('mt.msg.loaded_editor'), 1000);
		}
		...
		function RecordStart () {
			...
			var tr = activeTrack ();
			if (!tr) {
				OneUp (t('mt.msg.arm_record'), 1200);
				return ;
			}
			...
			if (!app.rec.startCapture ({
				...
				onstart: function () {
					...
					OneUp (t('mt.msg.recording') + ' ' + tr.name, 1000);
				},
				onerror: function () {
					if (rec === r) rec = null;
					OneUp (t('mt.msg.no_device'), 1200);
				}
			})) {
				rec = null;
				return ;
		...
		function RecordStop ( done ) {
			...
			app.rec.stopCapture (function () {
				...
				selected_clip = clip.id;
				pushState ( prev, t('mt.desc.record_clip') );
				render ();
				rec_el = null;
				rec_canvas = null;
				app.fireEvent ('DidUpdateMultitrack');
				app.fireEvent ('DidSelectClip', clip);
				OneUp (t('mt.msg.recorded'), 1000);
				done && done ( clip );
			});
		}
		...
		function deleteSelectedClip () {
			...
			selected_clip = null;
			app.fireEvent ('DidDeselectClip');
			pushState ( prev, t('mt.desc.delete_clip') );
			queuePlayRefresh ( true );
			render ();
			app.fireEvent ('DidUpdateMultitrack');
			return true;
		...
		function splitSelectedClip ( at ) {
			...
			if (rel <= 0.005 || rel >= len - 0.005) {
				OneUp (t('mt.msg.move_marker'), 1200);
				return false;
			}
			...
			pushState ( prev, t('mt.desc.split_clip') );
			queuePlayRefresh ( true );
			render ();
			app.fireEvent ('DidSelectClip', right);
			OneUp (t('mt.msg.split'), 900);
			return true;
		}
		...
		q.Propagate = function ( id, arg1, arg2 ) {
			...
			{
				OneUp (t('mt.msg.fx_editor'), 1400);
				return true;
			}
			if (id === 'RequestActionFXUI_Flip') {
				OneUp (t('mt.msg.channel_editor'), 1400);
				return true;
			}
		...
		app.listenFor ('StateDidPop', function ( state, undo ) {
			...
			if (undo) OneUp (t('mt.msg.undo') + ' ' + state.desc);
			else OneUp (t('mt.msg.redo') + ' ' + state.desc);
		});
